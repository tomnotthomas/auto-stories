...
describe('StoryController', () => {
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [
        ...
        JobService,
        // Only the guard needs it now; the budget is reserved in the generator.
        { provide: FairUseService, useValue: { enforceIp: jest.fn() } },
      ],
    }).compile();
  ...
  // The budget is now reserved inside the generator, at the model call itself
  // (7.37), so the controller's job is only to surface the refusal.
  it('fails the job with quota_exhausted when the generator refuses', async () => {
    generate.mockRejectedValue(ApiErrors.quotaExhausted());

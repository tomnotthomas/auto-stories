...
describe('kickerFor', () => {
  it('uses the model’s own kicker when it wrote one', () => {
    expect(kickerFor(frame('x', { order: 3, kicker: 'saturday' }), 6)).toBe('saturday');
  });

  it('falls back to the frame’s place in the story — never an invented reason', () => {
    expect(kickerFor(frame('x', { order: 1 }), 6)).toBe('opens the story');
    expect(kickerFor(frame('x', { order: 6 }), 6)).toBe('closes it');
    expect(kickerFor(frame('x', { order: 3 }), 6)).toBe('next beat');
    expect(kickerFor(frame('x', { order: 1 }), 1)).toBe('opens the story');
  });
  ...
  it('never claims a frame closes a story whose length it does not know yet', () => {
    ...
    expect(kickerFor(frame('x', { order: 1 }), null)).toBe('opens the story');
    expect(kickerFor(frame('x', { order: 4 }), null)).toBe('next beat');
  });
});

describe('typeFor', () => {
  it('sets the words at the end the story’s Look will anchor them to', () => {
    const type = typeFor(frame('Out before the light went'), 6, 'magazine-masthead');
    expect(['top', 'bottom']).toContain(type.position);
  });

  it('numbers the words across the whole headline so they land one at a time', () => {
    const type = typeFor(frame('It started with a mirror and a plan'), 6);
    expect(type.lines.map((line) => line.startIndex)).toEqual([0, 4]);
    expect(type.wordCount).toBe(8);
  });

  it('carries the kicker', () => {
    expect(typeFor(frame('The audit', { order: 6, kicker: 'the audit' }), 6).kicker).toBe(
      'the audit',
    );
  });

  it('keeps a silent frame silent — no words, and nothing to reveal', () => {
    const type = typeFor(frame('', { density: 'silent' }), 6);
    expect(type.wordCount).toBe(0);
    expect(type.silent).toBe(true);
  });

  it('is not silent when the model wrote words', () => {
    expect(typeFor(frame('The audit'), 6).silent).toBe(false);
  });
});
...
describe('revealDuration', () => {
  ...
  it('is shorter for a shorter headline', () => {
    const short = typeFor(frame('The audit'), 6);
    const long = typeFor(frame('It started with a mirror and a plan'), 6);

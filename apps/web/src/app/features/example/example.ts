...
import {
  ...
  DEFAULT_PLACEMENT,
  type FramePlacement,
} from '../refine/caption-editor/caption-editor';
...
const SEED: readonly Omit<ExampleFrame, 'placement' | 'legibility'>[] = [
  {
    src: 'sample/example-1.jpg',
    headline: 'Everyone made it to the lake for Maya’s first birthday.',
  },
  {
    src: 'sample/example-2.jpg',
    headline: 'The cousins claimed the dock before the cake even arrived.',
  },
  { src: 'sample/example-3.jpg', headline: 'Ten cousins, one birthday girl, zero chill.' },
  { src: 'sample/example-4.jpg', headline: 'Then it was time — one cake, one candle.' },
  {
    src: 'sample/example-5.jpg',
    headline: 'One candle, ten sets of hands ready to help her blow it out.',
  },
];
...
export class Example {
  ...
  protected onHeadline(headline: string): void {
    this.patchCurrent({ headline });
  }
  ...
  private patchCurrent(patch: Partial<ExampleFrame>): void {
    ...
    this.frames.update((frames) =>
      frames.map((frame, idx) => (idx === i ? { ...frame, ...patch } : frame)),
    );
  }

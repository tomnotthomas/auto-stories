...
- **Follow-up — dropped the proxy to 768px / JPEG 70% once the count rose to 30.** With the pick raised to 30 (see 4.2 follow-up), a full 30-photo batch began returning a **504** on the deployed app — Render's proxy has a **fixed ~100s request timeout that is not configurable**, and the single `POST /generate` (upload + one Gemini call) exceeded it. Confirmed it scaled with count, not cold start: 10 photos succeeded, and 30 still timed out ~5 min in (well inside the free plan's 15-min spin-down, so warm). Options: raise a timeout (none available — Render's is fixed, and our own Gemini cap is already 25s and fails gracefully, not a 504); async job + poll (correct, but not a now-fix); shrink the per-photo payload. Chose the last: **MAX_EDGE 1024 → 768, JPEG_QUALITY 0.8 → 0.7.** 768px is exactly one Gemini tile (not two), so it roughly halves both the upload bytes and the per-image model latency; combined with the quality drop the 30-photo payload falls to well under half, back inside the 100s window. 768px stays above the ~512px legibility floor. Async remains the proper fix for scale.
...
- **Problem:** A reviewer cloning the repo landed on the take-home *challenge prompt* (root `README.md`), not on how to run Auto Stories. The only run instructions were comments in `docker-compose.yml`/`Dockerfile`. Local dev was undocumented: it needs the API (`:3000`) and web (`:4200`, which proxies `/api`) started separately, discoverable only from `angular.json` + `proxy.conf.json`. Root `package.json` had only `typecheck` + `openapi:*`; no `dev`/`build`/`test`/`lint`.
- **Options:** (README) keep the prompt untouched and add a separate DEVELOPING.md; or replace the README with a project README and preserve the prompt. (dev script) document a two-terminal flow with no new dep; or add `concurrently` for a single `npm run dev`.
- **Decision:**
  - **README** = the Auto Stories project README (live URL, one-command Docker run, local dev + the proxy, layout, command table, docs links). The verbatim challenge prompt is renamed to `CHALLENGE.md` at the repo root (kept, not deleted).
...
- **Decision:** Option 3. A full-width "Add a photo" button sits in the refine action bar, above Reorder & remove / Regenerate. It reuses the filmstrip Add pipeline: pick → `addPhotos` → append each new photo as a captioned frame, no rebuild (5.16).
...
- **Follow-up — at the photo cap the button disables, it does not disappear.** First cut hid the button on `isFull` (pool ≥ `MAX_PHOTOS`). But after generation the pool still holds every picked photo, so picking the full 30 (e.g. the sample set) built a ~6-frame story yet hid "Add a photo" — and it only reappeared after dropping a frame (`dropPhoto` also frees a pool slot). A vanishing/reappearing control reads as a bug and gives no reason. Fix: always render the button; at the cap it is disabled with the hint "Max 30 photos — remove one to add another." The 30-cap (generation input size) is unchanged — the change is only making the limit legible instead of silent.

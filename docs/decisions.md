...
### 3.7 Latency UX
> Reversed by [7.29](#729-the-wait-shows-the-work-and-the-user-can-pull-a-photo-out-of-it-reverses-37) — the wait is 20–40s, not a few seconds, and the screen now shows the work instead of hiding it.
...
### 7.26 A designer and a content creator, sharing a brief instead of talking
- **Problem:** How much text a frame needs is a joint decision. Some moments want a reflective few lines, some want one word, some want none, and some want a question that pulls the viewer in. The model currently writes words with no idea how they will be set, so the amount is arbitrary — sometimes too much for the design, sometimes too little to tell the story. Framed as two parties: a **designer** who builds the frame and says where things go, and a **content creator** who fills it while keeping the story present. Neither can do the frame alone.
- **Options:** (a) a real back-and-forth — the two exchange proposals per frame until they agree; (b) a **shared brief** — the designer publishes what it can set at each level of text, the content creator picks one and writes to it, and the two meet deterministically in code; (c) leave it to the model alone.
- **Decision:** (b). The content creator picks a **density** per frame — `silent` (no text), `beat` (1–3 words), `line` (4–12 words), `thought` (2–3 lines, 15–35 words), `question` (one short question). The designer declares, per Look, the slot and type ramp for each density and how much fits. The resolution happens on the client: density picks the slot, and the Look re-ramps or trims if the words overflow. `question` is a different axis from the rest — the others are about *how much*, it is about *what for* — but it stays one rung of the same enum so the model cannot emit "silent + question", and so a Look can set a question differently from a statement. A `question` frame is what pairs with Instagram's poll/question sticker. If a genuine critique step is ever needed, it is **one pass over the whole story**, never one per frame.
- **Why:** (a) is what 7.23 already was — several model calls per frame — and it timed out on every story; it also breaks the story-level/frame-level split that makes frames safe to build in parallel. The designer is deterministic, so the content creator can be told in advance exactly what happens to each answer; a conversation buys nothing that a shared vocabulary does not. (c) is the status quo and produces the arbitrary amounts. **This supersedes part of 7.25 slice 1:** a frame with no words was made unusable and dropped, which is right only while every frame must carry text — once `silent` exists it is a legitimate editorial choice and the frame must survive. `thought` must be given a visibly different slot from `line`, or the model collapses the two. (Live review, 2026-08-03.)

### 7.27 Every story wears the loudest Look, because only one is built
- **Problem:** A Halloween-with-the-family story rendered cold and corporate, with the type dominating the photo — none of the warmth the moment called for. Two candidate causes: no Look in the set carries a heartfelt, warm quality, or the model's judgement about which Look to pick is wrong.
- **Options:** (a) add a warmer Look; (b) tune the Look-selection prompt; (c) find out which before changing anything.
- **Decision:** (c) first, and it was neither. The model's judgement is sound — asked for "loud chaotic bachelor party" it chose `bold-poster`, and for "grandmother's 90th, everyone crying and laughing" it chose `film-postcard`. The warm Looks already exist in the contract (Scrapbook, Film Postcard) and the restrained ones too (Minimal, Quiet Editorial). The cause is in `look.ts`: the registry implements **only** `magazine-masthead`, and `lookFor()` falls back to it for every other id, so all six choices render as the one Look — the board's "most overtly designed", carrying the largest type in the set (headline at 9.4% of frame width). Building the other five (Looks-engine plan P2) is therefore promoted ahead of the frame-harmony slices.
- **Why:** this was a known P1→P2 gap, flagged when Magazine shipped as "user-visible", but under-rated: seeing *every* story in the most corporate of the six is a different complaint from "one story looked off". It also explains the separate report that the font is too much in the forefront — not that type is inherently too loud, but that the loudest of the six is applied universally. Harmonious placement of a Look nobody wanted (slice 2) is worth less than having the right Look at all. Checking the registry cost nothing; the two generations that confirmed it were one more than needed, since the code alone was decisive. (Live review, 2026-08-03.)

### 7.28 Swipe the actions away, and never move the composition
- **Problem:** The three actions (Post to Instagram / Refine story / Start over) sit permanently on the photo, so the picture can never be looked at on its own. A first attempt added a labelled toggle to fold them; that fixed the obstruction but left ~48px of permanent chrome on the frame, and worse, folding **moved the composition** — a bottom-anchored headline slid down 144px, because the preview's bottom reservation was tied to whether the actions were shown.
- **Options:** (a) a labelled toggle; (b) restructure the preview to a true 9:16 frame so the reservation disappears; (c) **swipe to dismiss, with a constant reservation**.
- **Decision:** (c). Swipe down on the actions to dismiss, swipe up from the bottom edge to bring them back — the same place they left from, which is what makes them findable without leaving a control on the photo. A one-time hint covers the return gesture, which is the only part a user cannot discover by accident. The reservation is **constant** (`refining() ? REFINE_BAR_PX : ACTIONS_PX`) and no longer consults the fold state, so dismissing changes nothing but whether the overlay is drawn.
- **The gesture tracks the finger.** The panel follows the drag by `transform` and settles with a CSS *transition* (retargets when interrupted, where a keyframe would restart). Release completes on distance **or** a flick — 0.11 px/ms, the same threshold the spark drag used in 7.23. Past the boundary it rubber-bands rather than hitting a wall. Two things this exposed: velocity must be measured on the drag's **final segment**, not total travel, or a reversal reads as fast (a drag 120px down and back to 8px is a small delta over a short time) and flings the panel away exactly when the user changed their mind; and `event.timeStamp` is whole-millisecond in jsdom, so every flick measured as zero elapsed until it was replaced with `performance.now()`.
- **Why:** the composition moving is the real defect — chrome coming and going must not recompose the frame — and a constant reservation is the whole fix. (b) was proposed first on a **mis-diagnosis**: I claimed the preview reserved layout space for a bar, when the photo is a full-bleed `object-cover` image and the actions are a pure overlay with nothing behind them. Restructuring the frame would have cost the deliberate full-bleed look to solve a problem that did not exist. (a) contradicts its own purpose: permanent chrome on a photo you are trying to see unobstructed. A gesture needs two guards that a button does not — a drag still emits a `click`, so a swipe over "Start over" could destroy the story, and a gesture-only control is unreachable by keyboard or screen reader, so an off-screen focusable button carries the same action. (Live review, 2026-08-03.)

### 7.29 The wait shows the work, and the user can pull a photo out of it (reverses 3.7)
- **Problem:** Generation takes 20–40s and the screen spent it on a three-step ticker over the user's photos. [3.7](#37-latency-ux) called this settled and deferred anything more to Phase 3, on the premise that the wait is "only a few seconds". It isn't — a free-tier cold start plus the model call is most of a minute, and a ticker that names steps it is not actually observing is a progress bar with extra words. It is also the longest single stretch of attention the app ever gets.
- **Options:** (a) keep the ticker (3.7's answer); (b) a game or other distraction for the wait; (c) **show the work** — the user's own photos as prints on a light table, drifting past, piling up as they are looked at, with the model's choices caught and written on as they land.
- **Decision:** (c), and the premise of 3.7 is reversed: **the goal is no longer to hide the wait but to show it.** The user's real photos drift up the screen with depth from scale, blur and opacity; a print that reaches the top lands on a dim "looked at" pile; the model's first choice is caught, held still, and its words set on it. The user can grab any drifting print and **pull it down to keep it** or flick it up to pass. A photo they kept that the model did not use is appended to the story through the existing add-a-photo path. The kept pile then flattens into the story's own progress bars as the first frame opens full-bleed. Streaming stays out of this slice — one round trip, so only the first choice is read out in full. (Plan: [generating-experience-plan.md](phase-2/generating-experience-plan.md).)
- **Why:** the ticker's steps were not measurements — nothing reported them — so it was a spinner claiming to be a narrator. The photos on screen are the actual input to the actual job, so showing them is both honest and the only content available that the user cares about. It also makes the wait the app's introduction to what it does (it reads your photos and chooses) rather than dead time.
- **Words land one at a time, tracking tightening under them — not a clip-path wipe.** Each word resolves out of `blur(10px)` and drops `0.18em` to its baseline, 78ms apart, while the line's `letter-spacing` runs `.06em → -.03em` over 1100ms. **Why:** a wipe is one uniform gesture applied to the whole line — one duration, one direction, no relationship to the words — so it reads as a shape moving over text rather than text being set. Landing words one at a time and tightening the line is what a compositor's proof does, and it is the only part of the screen where the model's actual output appears, so it is where the craft has to be visible. Three earlier versions were rejected on this point alone.
- **Passed photos drift on; they are never crossed out.** **Why:** these are the user's own photos. An X, a strike-through or a red mark is the app judging them. Drifting past and dimming says "looked at", which is what actually happened.
- **The interaction is "pull a print down to keep it", not a minigame.** **Why:** a minigame concedes that the wait is boring and asks the user to do something unrelated to be distracted from it. Pulling a print down is the same job the model is doing at the same moment, so it holds attention on the product's actual value, and it produces something real — an explicit signal of intent that goes into the story. When the model lands on a photo the user already pulled down, that print is lifted back out of the pile and written on with "you called it" rather than a second copy being dealt.
- **Reduced motion keeps the reveal and the gesture and drops only the movement** — the drift, the depth blur and the travel go; opacity fades at 240–300ms stay. **Why:** the vestibular trigger is movement, not change; making everything instant removes the structure that explains what is happening and reads as a broken screen rather than an accessible one. (The same rule the shared motion utilities already follow.)

### 7.30 Each pick is shown as it is made — streamed where the model allows it, paced where it does not
- **Problem:** [7.29](#729-the-wait-shows-the-work-and-the-user-can-pull-a-photo-out-of-it-reverses-37) shipped with one round trip, so the model's choices were only known at the very end: the first was read out and the other five dropped onto the kept pile together. The screen showed *that* photos had been picked, never *a photo being picked*. Reported directly: "it only shows in the end, not in between".
- **Options:** (a) leave it — the frames genuinely aren't known earlier; (b) **stream** the response and show each frame as the model writes it; (c) **pace** the reveal so every choice is shown one at a time, however they arrive.
- **Decision:** both (b) and (c), because measurement showed (b) alone does not fix it. `generateContentStream` replaces `generateContent` — still one call and one `responseSchema`, read as it arrives — and each finished frame is reported to the client over the job's existing SSE stream. On top of that, the reveal takes a **pace**: every beat divides by it, so a run of waiting choices is fitted into a **6s budget** instead of being skipped. The first choice is always read at full speed.
- **The measurement that decided it:** against `gemini-flash-latest` with 8 photos, the model spends **~17s reading** and then writes the entire answer in **~640ms** — all seven frames inside one second. So streaming moves the picks from "one at the end" to "seven at the end". Pacing is what actually spreads them: measured end to end, the picks now land at 20.7s, 25.2s, 26.2s, 27.9s, 28.8s and 30.0s.
- **Why:** watching the model choose is the whole premise of the screen (7.29) — a batch of finished frames is a result, and a photo being lifted, held and written on is the work. Streaming is kept even though it is not what fixed today's timing: it costs one call either way, it is what makes the reveal correct for any model or payload whose writing is slower than its reading, and it means the client is never waiting on data the server already has. Pacing the beats rather than cutting them keeps the crafted reveal (7.29) intact — what gives way is the reading time, not the choreography.
- **What is streamed is advisory.** Reported frames go through the same `shapeFrames` as the final result, an unparseable entry is skipped, a photo the model invented is dropped, and the story on the terminal event stays authoritative. A report that lands after the job settles is ignored so it cannot resurrect a terminal state. Contract: `JobState.frames`, cumulative, present only while `processing`.
- **A frame cannot claim to close a story whose length is unknown.** While the model is still writing, the frame in hand may or may not be the last, so the positional kicker only names the opener. (Follows the same rule as 7.29: never state something the response does not support.)

### 7.31 A mark is sized to its line, not to its font
- **Problem:** An emphasised phrase drawn as a filled block ([7.24](#724-deterministic-looks-replace-model-emitted-geometry-supersedes-721723)'s `accent-block`) overlapped the line above it. Reported as "when there's a background colour to emphasise something, it collides with the line above and lays on top of it a little".
- **Cause:** a background painted behind an *inline* run covers the font's **content area** — ascent + descent, ~1.16em for Bricolage — and inline padding cannot grow a line box, so the extra `0.06em` of air had nowhere to go but over the neighbouring line. The two Looks that use a block (Bold Poster, Zine) set headlines at **0.88-0.94em** leading, which is tighter than the content area on its own. Measured at the real size: **57.4px of block in a 42.2px line**.
- **Options:** (a) loosen the leading on a marked line until the block fits (~1.28em); (b) drop the block's vertical air; (c) **size the block off the line** rather than off the font.
- **Decision:** (c). One shared rule (`blockMarkHeightEm`) that both renderers use: as tall as the line less a hairline of air, capped at 1em, floored so it always covers the caps. The DOM paints it as a sized background instead of a fill plus vertical padding; the canvas export draws the same height, centred in the line.
- **Why:** (a) changes the design to fit the implementation — 0.94em leading is what makes a poster headline a poster headline, and it would also mean a marked frame and an unmarked frame of the same story were set differently. (b) removes the air that makes the reversed type legible, and does not actually fix it: even with no padding the content area is still taller than the line. (c) leaves every Look's authored leading untouched and is font-independent — a face with a taller content area cannot reintroduce the bug. The rule lives in the composition vocabulary rather than in either renderer, so the preview and the exported PNG cannot drift ([7.25](#725-one-composition-owns-the-frame--nothing-else-draws-on-the-photo)).

### 7.32 The generating screen is watched, not operated (drops the keep gesture from 7.29)
- **Problem:** [7.29](#729-the-wait-shows-the-work-and-the-user-can-pull-a-photo-out-of-it-reverses-37) gave the user a gesture during the wait: grab a drifting print, pull it down to keep it, flick it up to pass. Called on review as not needed.
- **Options:** (a) keep it; (b) keep it but only on touch; (c) **remove it**.
- **Decision:** (c). Out go the drag, the flick, the drop zones and their armed states, the white "yours" dot, the "N yours" tally, the "drag a photo down to keep it" invitation, the agreement beat ("you called it"), and `StoryService.userPicks` with the append-my-picks wiring behind it. What stays is the whole of the reveal, plus the one tap that moves a beat along. The lane engine loses its gesture half — 554 lines to 394 — and the screen keeps doing the only job it was built for.
- **Why:** the screen's premise is *showing* the model choose, and a gesture is a second thing to notice while that is happening. It also earned its complexity twice over: a drag on a moving target needs pointer capture, multi-touch guarding, damping, flick velocity and a spring-back, and it was the one part of the screen that could go wrong under a finger. Removing it takes ~160 lines and 23 tests out of the app and leaves nothing the user has to work out. The picks it produced were never load-bearing — they were appended after the fact through the refine path, which is still there and still reachable deliberately.

### 7.33 On a phone the wait must still be doing something
- **Problem:** the generating screen "looks broken" on Android. Two defects in it, both invisible on a desktop browser.
- **Reduced motion left a dead screen.** [7.29](#729-the-wait-shows-the-work-and-the-user-can-pull-a-photo-out-of-it-reverses-37) said reduced motion keeps the fade and drops the movement — but this screen's entire content *is* movement, so dropping the drift left four photos sitting still for 20-40s. Android's battery saver turns `prefers-reduced-motion: reduce` on, so this is the ordinary state of a phone that is low on charge, not an edge case. **Fixed:** the lane still works through the photos, on a 1.6s beat, in place — one print at a time, changed by opacity alone, with the "N looked at" tally counting. The part that is vestibular-unsafe (travel, drift, depth blur) stays gone; the part that says the app is working stays.
- **The surface was measured once.** The lane's landmarks — where a print leaves the lane, where it is caught, where the piles sit — were fixed at the size the screen had when it mounted. On Android the URL bar collapses as you look at it, which changes the viewport height by ~10%, and everything on the table was then laid out for a screen that no longer existed. **Fixed:** the screen re-measures on resize; prints still in the lane keep their place proportionally, and prints on a pile are laid out again from the pile's own rule.
- **What this exposed twice:** a finished `fill: forwards` animation keeps owning the properties it animated, so it outranks anything written afterwards — the same trap as the [greyscale print](#) earlier. A resize has to take every element back from whatever is animating it before rewriting it.
- **Not changed:** the depth blur. It was the obvious suspect for jank, but measured at phone pixel density (DPR 3) it is free — 60fps with it and without it, frame for frame. Ripping out the depth on a guess would have cost the design for nothing.

### 7.34 The lane is cheap by default and sheds what a phone cannot afford
- **Problem:** the generating screen is choppy on Android — not stuck, just never fluent. It has to run on a cheap phone, and the expensive part of it is exactly the part that runs every frame.
- **What was actually costing:** (a) a **depth blur whose radius changes every frame**, over prints the size of the screen — a blur is re-rendered whenever its radius changes, and fill rate is where a budget GPU is slowest; (b) **full-resolution photos** — a 12MP original costs ~48MB of decoded bitmap to show in a phone-sized box, and eight of them is memory pressure a cheap phone answers with garbage collection, which is what choppy looks like; (c) a **compositor layer per print**, kept even for prints that had stopped moving.
- **Options:** (a) drop the depth blur everywhere; (b) branch on a device signal (pixel ratio, memory, user agent); (c) **make the lane cheap by default and let it shed the blur when the frames say it must**.
- **Decision:** (c), and all three costs addressed. The blur radius is **stepped to 0.5px** so the compositor can reuse what it drew instead of re-blurring on every sub-pixel change. Photos are **downscaled once to a display copy** (1440px long edge) before the first print is dealt — an order of magnitude fewer pixels for a camera original, at a size no phone screen can tell apart. A print gets a **compositor layer only while it is moving**. And the screen **watches its own frame times**: if the median of the last 20 frames misses the budget, the depth blur is dropped for the session, one way.
- **Why not a device signal:** there is no property that says "this phone is cheap". A budget Android reports the same pixel ratio as a flagship, `deviceMemory` is coarse and often absent, and a user-agent list is a maintenance liability that is wrong the day it ships. Frame times are the thing we actually care about, measured on the device we are actually on.
- **Why one way:** a screen that flickered between sharp and soft as the load moved would be worse than either state.

...
- **Problem:** The raw `openapi-typescript` output isn't hand-usable — a model type is buried as `components["schemas"]["GenerateRequest"]`, and it's littered with routing scaffolding (`get?: never`, `query?: never`) that isn't part of the data model. It also needs to be imported by both apps from one place.
- **Options:** import the generated file directly from each app by relative path; publish a versioned npm package; a local workspace package that both apps depend on.
